...
func NewPlayersService(sqliteClient *gorm.DB, redisClient *rueidis.Client, repo repository.PlayersRepositoryContract) *PlayersService {
...
func (s *PlayersService) GetNewTurn(pervTurn, cardsWinner, king, WasTheKingChanged string) string {
    if WasTheKingChanged == "true" {
        return king
    }
    if cardsWinner != "" {
        return cardsWinner
    }
    nextTurn, _ := strconv.Atoi(pervTurn)

    return strconv.Itoa(s.NextPlayerIndex(nextTurn))
}
