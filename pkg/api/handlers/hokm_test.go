...
func TestDetermineTurnAfterChoosingTrump(t *testing.T) {
    ctrl := gomock.NewController(t)
    defer ctrl.Finish()

    e := echo.New()

    c, rec := tests.ApiCall(e, "POST", "/game/:gameID/choose-trump", map[string]string{
        "gameID": "12345",
    }, map[string]string{
    ...
    mockGameRepo.EXPECT().GetGameInformation(gomock.Any(), gomock.Any()).Return(
        tests.NewGameDataBuilder().BeginningState().Build(), nil,
    )

    mockCardRepo := mocks.NewMockCardsRepositoryContract(ctrl)
    mockCardRepo.EXPECT().SetTrump(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
    ...
    if assert.NoError(t, h.ChooseTrump(c)) {
