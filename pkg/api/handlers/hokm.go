...
func (h *HokmHandler) PlaceCard(c echo.Context) error {
	...
	params := repository.PlaceCardParams{
		...
		WasTheKingChanged: gameInformation["was_the_king_changed"],
...
func (h *HokmHandler) GetMenuPage(c echo.Context) error {
	...
	_, e := h.playersService.PlayersRepo.SavePlayer(user, chatInstance)

	if e != nil {
		fmt.Println("Error in saving player:", e)
	}
